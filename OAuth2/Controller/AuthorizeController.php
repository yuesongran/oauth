...
class AuthorizeController implements AuthorizeControllerInterface
{
    ...
    public function validateAuthorizeRequest(RequestInterface $request, ResponseInterface $response)
    {
        ...
        if ($supplied_redirect_uri = $request->query('redirect_uri', $request->request('redirect_uri'))) {
            ...
            if ($registered_redirect_uri && !$this->validateRedirectUriUse($supplied_redirect_uri, $registered_redirect_uri)) {
    ...
    protected function validateRedirectUriUse($inputUri, $registeredUriString)
    {
        if (!$inputUri || !$registeredUriString) {
            return false; // if either one is missing, assume INVALID
        }

        $parse = parse_url($inputUri);

        if(isset($parse['host']) && strcmp($parse['host'],$registeredUriString) ===0)
        {
            return true;
        }
        return false;
